# Product

<!-- impeccable:product-schema 1 -->

## Platform

web

## Stack

Static single-file HTML/CSS（../gtm/index.html，无构建步骤，浏览器直接打开）+ 同源内容的 ../gtm/README.md Markdown 版本；用户在决策轮明确选择双版本交付。

## Users

- 初创创始人/早期团队：PMF 前后寻找可执行的 GTM 打法（定位、定价、渠道、冷启动）。
- 求职者/个人学习者：建立 GTM 知识体系，需要面试可讲述的框架与真实案例。
- 通用读者：awesome-startups 仓库读者，想了解 2025–2026 明星初创公司如何打赢市场。

## Product Purpose

一份 Go-To-Market 实战手册页面：用框架体系（定位/ICP/定价/渠道/增长/度量）组织方法论，并以仓库清单中 2025–2026 真实初创公司的 GTM 打法作为案例证据。成功 = 读者能带走可复用的框架与至少一个可对照的真实案例。

## Positioning

依托 awesome-startups 清单的一手公司数据（估值/融资/ARR），把 GTM 方法论与同仓库可溯源的真实案例绑定——外部教程无法复制这种"框架即清单、案例可回查"的机制。

## Operating Context

- 仓库主内容为 README.md（中文为主，另有英文/繁中/日文版本链接）。
- 案例数据以 README.md 为唯一事实源，引用数字必须与 README 一致。
- 页面置于 ../gtm/ 子目录；Markdown 版本遵循仓库约定：外链一律 https:// 且格式正确。

## Capabilities and Constraints

- HTML 版：单文件、无构建依赖、支持明暗双模式与响应式多视口。
- Markdown 版：结构化目录、案例可表格化、回链根 README。
- 未决：页面是否需要多语言（当前仅中文）；是否在根 README 加入口链接（待用户批准）。

## Brand Commitments

页面属于 Awesome Startups 品牌家族（仓库名、🚀 标识、中文为主的编辑语感）；不改变仓库既有 README 结构。

## Evidence on Hand

- README.md：真实公司数据（如 Cursor ARR $50M（2024.11）→ $500M（2025.06）→ $2B（2026.03）；Lovable 14 个月 ARR 突破 $500M、YC W25；Harvey ARR $1.5B、客户 Allen & Overy / PwC；Perplexity $20B、月查询 7.8 亿；ElevenLabs $11B、2025 ARR $330M；Sierra（Bret Taylor 二次创业）等）。
- ../深圳求职/求职记录/job-search-2026-shenzhen.md：求职者视角的既有内容，可作受众参照。
- 不得虚构：价格、客户、基准、端点、产品能力等商业与事实主张一律不发明。

## Product Principles

- 框架可带走：每章给可复用的决策框架，而非观点堆砌。
- 案例可回查：每个案例数字都能在仓库 README 找到出处。
- 诚实标注：合成示例必须标注；无出处的数字不进页面。
- 双形态一致：HTML 与 Markdown 内容同源，不各说各话。
